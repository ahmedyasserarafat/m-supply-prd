...
public class javaDateTimeApi {
	...
	public static void main(String[] args) {
		...
		LocalDate jj=gg.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
		 //System.out.println(jj);
		//TimeZone.getTimeZone("UTC")
		LocalDateTime dt=LocalDateTime.now(ZoneId.of("GMT"));
		
		//Date to=Date.from(jj.toInstant(ZoneOffset.UTC));
		String hhhh=LocalDateTime.now().minusDays(1).truncatedTo(ChronoUnit.MINUTES).format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
		System.out.println("hhhhhhhhhh--"+LocalDate.now());
	}
