...
/**
 * This class is used to create an appender pattern for Registration logging.
 * 
 * Based on the roll over model set in the configuration file
 * currently supports 2 models
 * 1. CREATENEW - current logging would happen in the file name having the current date pattern and higher rolling index value. (i.e., logging would happen by creating a new file)
 * 2. BACKUPOLD - current logging would always happen in file name without date pattern and rolling index on roll over the current file would be made as a backup with a date pattern and index included in it.
 * 
 * The  class extends FileAppender class of log4j. 
 * This appender receives the following attributes from the configuration file
 * 1. layout, 
 * 2. appender file, 
 * 3. maxFileSize, 
 * 4. datePattern, 
 ...
 * and
 * 
 * 'datePattern' attribute: 
 * the appender expects date and time pattern as listed below 
 * which actually decides on the Roll over period whether to be monthly, weekly, daily etc. 
 * And the datePattern also decides on the format of date to be appended in the file name construction
 ...
 * It actually decides on the maximum file size of a log file considering the rollover period.
 ...
 * 
 * 'maxBackupIndex' attribute:
 * applicable only in case if roll over model is set as 'BACKUPOLD'. this value decides on the maximum number of back up files that could be created for a particular date pattern set.
 * 
 * 'rolloverModel' attribute:
 * expects one of the following 2 roll over models currently supported CREATENEW, BACKUPOLD, default being 'CREATENEW'
 * 
 *
 */
...
public class DailyRollingFileAndSizeAppender extends FileAppender 
{
	...
	/**
	 * Sets the rolloverModel option, which decides on the rollover model if configured in the configuration file (log4j.properties).
	...
	/**
	 * Sets the datePattern if configured in the configuration file (log4j.properties).
	 * which also determines the rolling period to be daily, monthly or weekly based on the input.
	...
	/**
     *  Gets the datePattern that helps to decides on the roll over period and format of the date time stamp of the log file name.
	...
	/**
	 * Sets the suffixOrPrefix option, which decides on the location of the date/time stamp plus rolling index in reference to the .log extension if configured in the configuration file (log4j.properties).
	...
	/**
	 *  Gets the suffixOrPrefix that helps to decide on the location of the date/time stamp plus rolling index in reference to the .log extension
    ...
    /**
     * Instantiate a <code>DailyRollingFileAndSizeAppender</code> and open the
     * file designated by <code>filename</code>. The opened filename will
     * become the output destination for this appender given with the appender value
     *  @param  layout: layout type 
     *  @param  filename: name of the file of type String  
     *  @param  append: If the append parameter is true, the file will be appended to. Otherwise, the file designated by filename will be truncated before being opened.
     *  @throws IOException
     *  This API/logic has been take from org.apache.log4j.DailyRollingFileAppender
     */
     ...
     *  This method is overridden by FileAppender.
     *  This API/logic has been take from org.apache.log4j.DailyRollingFileAppender
     *  @see org.apache.log4j.DailyRollingFileAppender.setQWForFiles(Writer writer) 
     */
    ...
    /**
     * This method differentiates DailyRollingFileAndSizeAppender from its super
     * class.Actual writing occurs here.   
     * <p>Before actually logging, this method will check whether it is
     *  time to do a roll over. If it is, it will schedule the next
     *  roll over time and then roll over.
     *  @param Logevent when log should append
     *  This API/logic has been taken from org.apache.log4j.DailyRollingFileAppender
     */
    ...
    /**
     * Overriding setFile method of a FileAppender to create file format specific
     * to intellectOnline.It gets the latest rollingIndex for a file by help of findRecentIndex()
     * Creates file in format filename_daystamp_rollingIndex.extension     
     ...
     *  This API/logic has been taken from org.apache.log4j.FileAppender and modified so as to get a rolling
	 *  index on size increase beyond the specified limit and a day stamp for each day 
     */
    public synchronized void setFile(String filenameSet, boolean append, boolean bufferedIO, int bufferSize)throws IOException 
    {
    ...
    /**
     *  This method performs the initial activation options. Calls the super method
     *  Please Refer org.apache.log4j.FileAppender.activateOptions()
     *  This API/logic has been taken from org.apache.log4j.DailyRollingFileAppender
     */
     ...
     *  @param: type, indicates when appended file has to rolled out apart from max file size exceeds
     *  This API/logic has been taken from org.apache.log4j.DailyRollingFileAppender
     */
     ...
     * epoch+nextMillis(type) formatted according to datePattern. All date
     * formatting is done in GMT and not local format because the test
     * logic is based on comparisons relative to 1970-01-01 00:00:00
     * GMT (the epoch). (Refer: org.apache.log4j.DailyRollingFileAppender.computeCheckPeriod() 
     *
     *  @return : type,  returns the int value when appended file has to rolled out, apart from exceeds max file size  
     *  This API/Logic has been taken from org.apache.log4j.DailyRollingFileAppender.computeCheckPeriod() 
     */
    ...
    /**
     *	Given a filename this method returns filename without extension
     *  Ex: if you are passed the file name like ServerLog.log then this method will return only file name
    ...
    /**
     *	Given a filename this method returns extension.If no extension returns empty String
     *  Ex: if you are passed the file name like ServerLog.log then this method will return only extension
     *  @param targetFile - file name with extension
  	   ...
  	   // close current file, and rename it to scheduledFilename
  	      ...
  	      if (target.exists()) {
  			...
  			File file = new File(fileName);
  			  ...
  			  if(this.getMaxBackupIndex() > 0) {
  				  // Delete the oldest file
  				actFile = new File(constructFileName(fileName, this.getMaxBackupIndex(), sdf.format(new Date(file.lastModified()))));
  				  if (actFile.exists())
  				  ...
  				  for (int i = this.getMaxBackupIndex() - 1; i >= 1 && renameSucceeded; i--) {
  					actFile = new File(constructFileName(fileName, i, sdf.format(new Date(file.lastModified()))));
  					  if (actFile.exists()) {
  						tempTarget = new File(constructFileName(fileName, i + 1, sdf.format(new Date(file.lastModified()))));
    ...
    /**
     * This method is to get the rollingIndex for the latest file formed with the same timestamp.
     * first it will change given file name by appending current time stamp and <code>index</code> value. 
     * i.e if ServerLog.log is the given as file name then file name will changes to ServerLog_03Apr2007_1.log, 
     * then checks weather named file exists or not, if exists then continues the loop,
     * till file not exist, for suppose Server_03Apr2007_10.log is not exist means, rest 9 versions are existed in file system,
     * Then current rollover index is 10, it will assigns to rollingIndex     
    ...
    /**
     * This method is for parsing of filename .i.e, attach the day stamp and rolling index to the filename.
     * If there is an extension then the extension should be separated then day stamp and backup index
     * should be attached to the filename ,then the extension.
     *  Ex: if the file name is c:\S\applogs\ServerLog.log, then this method will parse that to c:\S\applogs\ServerLog_20070403_21.log
     *  In that 20070403(yyyyMMdd) is current time stamp, then rolling index,default rolling index is 1
     *  timestampSeparator
    ...
    /* It holds server status, when new instance being created for this class then I’m assuming server is restarted. That time I’m changing value to TRUE from FALSE in constructor*/
...
/**
 *  RollingCalendar is a helper class to DailyRollingFileAndSizeAppender.
 *  Given a periodicity type and the current time, it computes the
 *  start of the next interval.
 *  This class has been taken from org.apache.log4j.DailyRollingFileAppender
 */
...
{    
    ...
    /**
     * This method typically checks periodicity type and Adds the specified (signed) amount of time to the given time field,
